...
class HttpParser(object):
    ...
    def process_line(self, data):
        ...
        if self.type == HTTP_REQUEST_PARSER:
            ...
            url = urlparse.urlsplit(line[1])
            # logger.debug('original url is %s' % str(self.url))
            global BASEURL
            if url.query:
                url = urlparse.parse_qs(url.query)
                url = url['url'][0]
                url = kodi_utils.b64decode(url)
            elif BASEURL:
                url = '{}{}'.format(BASEURL, url.path)
            else:
                raise Exception('Got relative/invalid url without base path')
            self.url = urlparse.urlsplit(url)
            BASEURL = '{}://{}{}'.format(self.url.scheme, self.url.netloc, '/'.join(self.url.path.split('/')[:-1]))
            logger.debug('set baseurl to %s' % str(BASEURL))
            self.version = line[2]
        else:
            self.version = line[0]
            self.code = line[1]
    ...
    def process_header(self, data):
        if len(data) == 0:
            ...
            #Override user agent
            if key == 'User-Agent':
                value = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.143 Safari/537.36'
...
class Proxy(Thread):
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            try:
                ...
                logger.exception('error in app_proxy')
    ...
    def run(self):
        ...
        try:
            ...
            logger.exception('Exception while handling connection %r' % (self.client.conn))
        finally:
...
class TCP(object):
    ...
    def __init__(self, hostname='0.0.0.0', port=globals.PROXY_PORT, backlog=100):
...
if __name__ == "__main__":
    proxy = ProxyService(HTTP("0.0.0.0", globals.PROXY_PORT))
    # proxy.start()
    proxy.run()