...
def checksub():
   ...
   html = net.http_POST('http://'+THESITE+'/payments/login',data,headers).content
   ...
   if 'true' in html:
      ...
      html = net.http_GET('http://'+THESITE+'/payments/member').content
    ...
    if 'please contact website administator' in html:
       ...
       addDir('------>>>>>> [COLOR cyan]HTTP://'+THESITE+'[/COLOR] <<<<<<-----','',2007,'','','','')
       addDir('','',2007,'','','','')
    else:     
       dialog = xbmcgui.Dialog()
...
def CATEGORIES():
    ...
    if os.path.exists(cookie_jar) == True:
        ...
        addDir('[COLOR red].View Calendar[/COLOR]','http://sportsnationhd.tv/view.php?pg=calendar',2005,MEDIA_URL+'calendar.png','','','')
        addDir('[COLOR red].View USA Calendar[/COLOR]','http://sportsnationhd.tv/view.php?pg=usacalendar',2005,MEDIA_URL+'test.png','','','')
        addDir('[COLOR gold].Willows Sports Mix (Football Rugby & More)[/COLOR]','http://sportsnationhd.tv/apisprotected/extrassnhd.php',2003,MEDIA_URL+'extra.png','','','')
        ...
        addDir('[COLOR cyan].Sports On Demand[/COLOR]','http://sportsnationhd.tv/apisprotected/channels1.php',2003,MEDIA_URL+'cup.png','','','')                
...
def OnDemand(url):
    addDir('[COLOR lime]Search[/COLOR]','url',3000,'','','','','')
...
def CheckUpdate():
    dialog = xbmcgui.Dialog()
    link=OPEN_URL('https://raw.githubusercontent.com/Willowsportsmania/SportsMania/master/plugin.video.sportsnationhdtv/addon.xml')
    match=re.compile('name="SportsNationHD" version="(.+?)"').findall(link)[0]
