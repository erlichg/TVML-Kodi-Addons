...
def GetSeriesList(id, name):
	...
	matches = re.compile('<div class="t1677_menuitem_title">(.+?)</div>.+?<div class="t1677_Menu" onclick=".+?\'(.+?)\'.+?img src="(.+?)"', re.S).findall(matches[0])
	for name, url, iconimage in matches:
...
def GetSectionsList(url, iconimage):
	try:
		...
		print 'Failed to getSectionsList for {}'.format(url)
