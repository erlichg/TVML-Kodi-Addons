...
STVBINI = os.path.join(ADDON.getAddonInfo('path'), 'resources', 'ultimatemania.ini')

try:
   filename = 'ultimatemania.ini' 
   import xbmcvfs
   ottv = xbmcaddon.Addon('script.tvguidedixie')
   path = ottv.getAddonInfo('profile')
   file = os.path.join(path, 'ini', filename)
   if not xbmcvfs.exists(file):
       stvb = xbmcaddon.Addon('plugin.video.ultimatemania')
...
def checksub():
   ...
   html = net.http_POST('http://ultimatemania.rocks/payments/login',data,headers).content
   ...
   if 'true' in html:
      ...
      html = net.http_GET('http://ultimatemania.rocks/payments/member').content
      ...
      try:
          ...
          addDir('@','',2007,'','','','')
          addDir('WWW.ultimatemania.rocks','',2007,'','','','')
          addDir('','',2007,'','','','')
    ...
    if 'please contact website administator' in html:
       ...
       addDir("------>[COLOR red]Please Contact an Admin to Unlock Account[/COLOR]<-----",'',2007,'','','','')
       addDir('','',2007,'','','','')
       addDir("------>[COLOR red]Your Account has been locked[/COLOR]<-----",'',2007,'','','','')
       addDir('','',2007,'','','','')
       ...
       addDir('------>[COLOR cyan]HTTP://ultimatemania.rocks[/COLOR]<-----','',2007,'','','','')
       addDir('','',2007,'','','','')
    else:     
       dialog = xbmcgui.Dialog()
...
def Login():
    #print '###############    LOGIN TO UltimateMania   #####################'
    loginurl = 'http://'+THESITE+'/view.php?pg=login'
    username = ADDON.getSetting('snusername')
    password = ADDON.getSetting('snpassword')
    ...
    data={'vb_login_username':username,'vb_login_password':password,'s':'','do':'login','vb_login_md5password':'','vb_login_md5password_utf':'','url':'http://ultimatemania.rocks/forum.php'} 
  

    headers={'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
    'Host':THESITE}
    ##b=net.http_GET('http://ultimatemania.rocks/payments/login').content
    html = net.http_POST('http://ultimatemania.rocks/login.php', data, headers).content
  
    net.http_GET('http://www.ultimatemania.rocks/payments/member/index')
    ...
    a=net.http_GET('http://ultimatemania.rocks/matrixuplink.php?do=getStreamChannels').content
...
def CATEGORIES():
    ...
    if os.path.exists(cookie_jar) == True:
        ...
        li = "47"
        if li in lista:
            addDir('[COLOR red].View Calendar[/COLOR]','http://ultimatemania.rocks/view.php?pg=calendar',2005,MEDIA_URL+'calendar.png','','','')
            addDir('[COLOR aqua].Maintenance Tools[/COLOR]','url',16000,'','','','')
            addDir('[COLOR red].View USA Calendar[/COLOR]','http://ultimatemania.rocks/view.php?pg=usacalendar',2005,MEDIA_URL+'test.png','','','')
            addDir('[COLOR gold].Willows Sports Mix (Football, Rugby & More)[/COLOR]','http://'+THESITE+'/apisprotected/extrasum.php',2003,MEDIA_URL+'extra.png','','','')
...
def comingSoon():
    dialog = xbmcgui.Dialog()
    dialog.ok(THESITE.upper(), 'COMING SOON',"This section is coming soon!", "Try Another Channel")
...
def CheckUpdate():
    dialog = xbmcgui.Dialog()
    link=OPEN_URL('https://raw.githubusercontent.com/Willowsportsmania/SportsMania/master/plugin.video.ultimatemania/addon.xml')
    match=re.compile('name="UltimateMania" version="(.+?)"').findall(link)[0]
